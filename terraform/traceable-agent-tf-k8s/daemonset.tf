resource "kubernetes_daemonset" "traceable-agent" {
  count      = var.ebpf_only == false && var.run_as_daemon_set == true ? 1 : 0
  ...
  # Note: The daemonset spec is very similar to the deployment spec. The deployment spec adds replica
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        host_network         = local.daemon_set_mirroring
        dns_policy           = local.daemon_set_mirroring ? "ClusterFirstWithHostNet" : "ClusterFirst"
        ...
        container {
          ...
          dynamic "env" {
            for_each = length(var.refresh_token_file) == 0 && local.bootstrap_refresh_token_from_gcp != true && local.use_external_token_secret != true ? [1] : []
            content {
          ...
          dynamic "volume_mount" {
            for_each = local.mirroring_enabled == true ? [""] : []
            content {
        ...
        dynamic "container" {
          for_each = local.mirroring_enabled == true ? [""] : []
          content {
            image             = "${local.image_registry_with_suffix}/suricata${local.suricata_image_separator}${var.suricata_version}"
            image_pull_policy = var.image_pull_policy
            ...
            dynamic "port" {
              for_each = var.pod_mirroring_enabled == true ? [""] : []
              content {
                container_port = 4789
              }
            }
